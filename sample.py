...
def plot_mel_auto(
    ...
    if mode == "auto" and expected_sec is not None:
        # pick the candidate whose time range is closer to expected
        if abs(t_x - expected_sec) <= abs(t_y - expected_sec):
            pick = "time-x"
        else:
            pick = "time-y"
    else:
...
# ---------------- main ----------------
def main():
    ap = argparse.ArgumentParser(description="CKPT-only ascent; normalize ONLY original; auto axes per image")
    ...
    ap.add_argument("--axes", type=str, default="auto", choices=["auto","time-x","time-y"],
                    help="auto picks orientation so time axis matches expected duration")
    args = ap.parse_args()
    ...
    # Per-image color scaling (prevents “washed out” look)
